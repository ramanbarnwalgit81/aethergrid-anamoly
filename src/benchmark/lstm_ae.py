...
def train_lstm_ae(X_train: np.ndarray, seq_len: int = 24,
    ...
    for epoch in range(n_epochs):
        ...
        model.eval()
        with torch.no_grad():
            # Mini-batch the validation forward pass so a large pooled validation
            # set (e.g. Farm C, 952 features) does not allocate the whole tensor
            # at once and exhaust memory. Per-element mean is preserved.
            vb = 512
